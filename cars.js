...
class Car {
    ...
    calculateTargetPosition() {
        // Calculate proper exit point based on turn type to ensure correct lane usage
        const exitPoint = this.intersection.getProperExitPoint(this.fromDirection, this.toDirection, this.turnType);
        this.targetX = exitPoint.x;
        this.targetY = exitPoint.y;
        
        // If turning, calculate the turning path
        if (this.turnType !== CONFIG.TURN_TYPES.STRAIGHT) {
            this.turningPath = this.intersection.getTurningPath(this.fromDirection, this.toDirection, this.turnType);
        }
    }
    ...
    updateCrossing(dt) {
        ...
        if (this.turnType !== CONFIG.TURN_TYPES.STRAIGHT && this.isInIntersection) {
            ...
            if (distance < 10) {
                // We've reached the target position, now change our movement direction
    ...
    moveToTargetPosition(dt) {
        // Move directly towards target position when turning
        const dx = this.targetX - this.x;
        const dy = this.targetY - this.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
...
export class CarManager {
    ...
    spawnCar() {
        ...
        if (!tooClose) {
            const car = new Car(this.nextCarId++, direction, this.intersection);
            this.cars.push(car);
        }
    }
