...
export class TrafficLightController {
    ...
    render(ctx, intersection) {
        Object.entries(this.lights).forEach(([direction, light]) => {
            this.renderTrafficLight(ctx, parseInt(direction), light.state, intersection);
        });
    }

    renderTrafficLight(ctx, direction, state, intersection) {
        const position = intersection.getLightPosition(direction);
        const lightSize = CONFIG.LIGHT_SIZE;
        const spacing = lightSize + 2;

        // Draw light pole
        ctx.fillStyle = '#666666';
        ctx.fillRect(position.x - 2, position.y, 4, CONFIG.LIGHT_POLE_HEIGHT);
        ...
        lights.forEach((color, index) => {
            ...
            // Light background
            ctx.fillStyle = CONFIG.LIGHT_COLORS.OFF;
            ctx.beginPath();
            ctx.arc(position.x, lightY, lightSize, 0, Math.PI * 2);
            ctx.fill();

            // Active light
            if ((color === 'red' && state === CONFIG.LIGHT_STATES.RED) ||
                (color === 'yellow' && state === CONFIG.LIGHT_STATES.YELLOW) ||
                (color === 'green' && state === CONFIG.LIGHT_STATES.GREEN)) {
                
                ctx.fillStyle = CONFIG.LIGHT_COLORS[color.toUpperCase()];
                ctx.beginPath();
                ctx.arc(position.x, lightY, lightSize - 1, 0, Math.PI * 2);
                ctx.fill();

                // Add glow effect
                ctx.shadowColor = CONFIG.LIGHT_COLORS[color.toUpperCase()];
                ctx.shadowBlur = 6;
                ctx.beginPath();
